...
/**
 * An implementation of Ryu (a floating-point to String conversion algorithm) for double.
 */
public final class RyuDouble {
	...
	public static String doubleToString (double value, int scientific) {
		StringBuilder sb = new StringBuilder(24);
		buildDouble(sb, value, scientific);
		return sb.toString();
	}

	public static void buildDouble (StringBuilder builder, double value, int scientific) {
		...
		if (e2 >= 0) {
			...
			if (DEBUG) {
				long exact = POW5_INV[q].multiply(BigInteger.valueOf(mv)).shiftRight(-e2 + q + k).longValue();
		...
		if (dmIsTrailingZeros || dvIsTrailingZeros) {
			...
			output = dv + ((dv == dm && !(dmIsTrailingZeros && /*roundingMode.acceptLowerBound*/ (even))) || (lastRemovedDigit >= 5) ? 1 : 0);
		} else {
			while (dp / 10 > dm / 10) {
				if ((dp < 100) && scientificNotation) {
					// Double.toString semantics requires printing at least two digits.
